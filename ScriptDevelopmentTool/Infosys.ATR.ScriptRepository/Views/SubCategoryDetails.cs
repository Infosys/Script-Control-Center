            ...
            try
            {
                if (_newItem)
                {
                    if ((int)cmbCategories.SelectedValue >= 0)
                    {
                        ...
                        if (client.ServiceChannel.AddCategory(req).IsSuccess)
                        {
                            ...
                            MessageBox.Show(string.Format("The new sub category- {0} is added.", cat.Name), "Done", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                        {
                            string err = "There is an error adding the sub category. \nPlease verify if the details provided are correct and name is not same as any existing Sub Category under the same Category.";
                ...
                else
                {
                    ...
                    if (client.ServiceChannel.UpdateCategory(req).IsSuccess)
                    {
                        ...
                        MessageBox.Show(string.Format("The sub category- {0} is updated.", subcat.Name), "Done", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        string err = "There is an error updating the sub category. \nPlease verify if the details provided are correct and name is not same as any existing Sub Category under the same Category.";
                        MessageBox.Show(err, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
