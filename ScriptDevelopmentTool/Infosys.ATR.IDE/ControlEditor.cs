...
namespace Infosys.ATR.DevelopmentStudio
{
    public partial class ControlEditor : Form
    {
            ...
            if (splitContainer1.Panel2.Controls.Count > 0)
            {
                if (splitContainer1.Panel2.Controls[0].GetType() == typeof(EditObjectModel))
                {
                    ...
                    if (ctl.Images != null)
                    {
                        ...
                        view.Set("Validate", ctl.Images.Validation);
                    }
                }
        ...
        void view_OnCapture(System.Drawing.Image image, string boundary)
        {
            ...
            if (temp.Images == null)
            {
                temp.Images = new CtrlImage();
                directory = objectModelPath;
                filename = element.DisplayText;

            }
            if (!String.IsNullOrEmpty(temp.Images.Center))
            {
                filename = Path.GetFileNameWithoutExtension(temp.Images.Center);
                directory = Path.GetDirectoryName(temp.Images.Center);
            }
            ...
            switch (boundary)
            {
                ...
                case "Center":
                    if (String.IsNullOrEmpty(temp.Images.Center))
                    {
                        t = Path.Combine(directory, filename + ".jpg");
                        image.Save(t);
                        temp.Images.Center = t;
                    }
                    else
                    {
                ...
                case "Validate":
                    if (String.IsNullOrEmpty(temp.Images.Down))
                    {
                        t = Path.Combine(directory, filename + "_validate.jpg");
                        image.Save(t);
                        temp.Images.Validation = t;
                    }
                    else
                    {
                        File.Delete(temp.Images.Validation);
                        image.Save(temp.Images.Validation);
                    }
                    break;
            }
            Save();
        ...
        private void ShowObjectModelView(ObjectModelViewMode mode)
        {
            ...
            switch (mode)
            {
                ...
                case ObjectModelViewMode.EditMode:
                    ...
                    view2.OnAdd += new AddNode(view_OnAdd);
