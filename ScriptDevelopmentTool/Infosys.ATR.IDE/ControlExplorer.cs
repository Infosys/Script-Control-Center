...
{
    public partial class ControlExplorer : Form
    {
        ...
        public static Desktop desktopElement = null;
        ...
        private TreeNode ExpandTheApplicationNode(int processid)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < trAppControl.Nodes[0].Nodes.Count; i++)
                {
                    ...
                    if (appElement.Current.ProcessId == processid)
                    {
            ...
            {
                if (MessageBox.Show("Caught by an error, do want to restart the control explorer?", "OOPS... Caught by an error", MessageBoxButtons.YesNo, MessageBoxIcon.Error) == System.Windows.Forms.DialogResult.Yes)
                {
                    this.Close();
        ...
        private void toolStripbtnCtrlCapture_Click(object sender, EventArgs e)
        {
            if (capturingOnMOve)
            {
            ...
            if (!capturingOnClick)
            {
                ...
                GlobalEventHandler.KeyUpEvents += new KeyEventHandler(HookManager_KeyUp);
                GlobalEventHandler.MouseDownEvents += new MouseEventHandler(HookManager_MouseDown);
                capturingOnClick = true;
                //toolStripbtnCtrlCapture.Text = "Stop capturing the control details from application";
                //toolStripbtnCtrlCapture.Image = imageList2.Images[1];
            }
            else
            {
                GlobalEventHandler.KeyDownEvents -= new KeyEventHandler(HookManager_KeyDown);
                GlobalEventHandler.KeyUpEvents -= new KeyEventHandler(HookManager_KeyUp);
                GlobalEventHandler.MouseDownEvents -= new MouseEventHandler(HookManager_MouseDown);
                capturingOnClick = false;
                //toolStripbtnCtrlCapture.Text = "Start capturing the control details from application. Press 'SHIFT + Mouse left key' on the control";
                //toolStripbtnCtrlCapture.Image = imageList2.Images[0];
            }
        }
        ...
        private bool InstantiateObjectModel()
        {
            if (desktopElement == null)
                return true;
            else
            {
                var app = desktopElement.Applications[0].Children[0];
                var winCtrl = app.ElementAttributes as WindowControl;
                var element = this.trAppControl.SelectedNode.Tag as UIElement ;
                if (winCtrl.ProcessId == ((WindowControl)element.ElementAttributes).ProcessId)
                    return false;
            }
            return true;
        }
        ...
        private void toolStripButton_SaveOM_Click(object sender, EventArgs e)
        {
            if (!menuStripDisabledForEdit)
            {
                ...
                if (!string.IsNullOrEmpty(objectModelPath))
                {
                    ...
                    if (toolStripComboBox1.SelectedItem.ToString() != "Web")
                    {
                        desktopElement = null;                        
        ...
        private void SaveAppObjectModel(TreeNode appNode, string destination)
        {
            ...
            if (copyImageToRespectiveFolders)
            {
                ...
                if (fileNameParts.Length > 0)
                {
                    ...
                    app.HierarchicalImageLoc = path + "\\" + CreateFileFolderName(fileNameParts[fileNameParts.Length - 1], 10) + DateTime.Now.Ticks.ToString()+ ".jpg";
            ...
            {
                desktopElement.Applications[0].Children.Add(app);
            }
        }
