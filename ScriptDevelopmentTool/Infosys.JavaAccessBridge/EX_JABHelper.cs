...
namespace Infosys.JavaAccessBridge
{
    public static class EX_JABHelper
    {
        public class AccessibleTreeItem
        {
            ...
            //added to understand the accessible actions
            public string accessibleActions;
        ...
        public static AccessibleTreeItem GetComponentTree(IntPtr hWnd, out Int32 vmID)
        {
            ...
            vmID = 0;
            if (JABHelper.isJavaWindow(hWnd) == 1)
            {
                unsafe
                {
                    IntPtr acPtr;

                    if (JABHelper.getAccessibleContextFromHWND(hWnd, out vmID, out acPtr))
                    {
        ...
        public static AccessibleContextInfo GetAccessibleContextInfo(Int32 vmID, IntPtr ac)
        {
            ...
            try
            {
                ...
                if (JABHelper.getAccessibleContextInfo(vmID, ac, acPtr))
                {
        ...
        public static AccessibleTextItemsInfo GetAccessibleTextInfo(Int32 vmID, IntPtr ac)
        {
            ...
            //Call DLL.
            JABHelper.getAccessibleTextItems(vmID, ac, ati, 0);
            //Creat object
        ...
        private static AccessibleTreeItem GetAccessibleContextInfo(Int32 vmID, IntPtr currentPtr, out AccessibleContextInfo currentContext, AccessibleTreeItem parentItem, int level, string lineage)
        {
            unsafe
            {
                ...
                try
                {
                    ...
                    Marshal.StructureToPtr(new AccessibleContextInfo(), acPtr, true);
                    if (JABHelper.getAccessibleContextInfo(vmID, currentPtr, acPtr))
                    {
                        ...
                        {
                            AccessibleTreeItem newItem = BuildAccessibleTree(currentContext, parentItem, currentPtr);
                            ...
                            if (!ReferenceEquals(newItem, null))
                            {
                                ...
                                if (currentContext.accessibleText == true)
                                {
                                    ...
                                    //assign the pointer to be used later to fetch actions
                                    //newItem.ElementPointer = (int)currentPtr;
                                ...
                                //check if the current object has any accessible actions
                                //if (currentContext.accessibleAction)
                                //{
                                //    AccessibleActions actions = new AccessibleActions();
                                //    IntPtr ptrActions = Marshal.AllocHGlobal(Marshal.SizeOf(new AccessibleActions()));
                                //    Marshal.StructureToPtr(new AccessibleActions(), ptrActions, true);
                                //    bool result = JABHelper.getAccessibleActions(vmID, currentPtr, ptrActions);
                                //    if (result)
                                //    {
                                //        actions = (AccessibleActions)Marshal.PtrToStructure(ptrActions, typeof(AccessibleActions));
                                ...
                                for (int i = 0; i < currentContext.childrenCount; i++)
                                {
                                    ...
                                    if (currentContext.role_en_US != "unknown" && currentContext.states_en_US.Contains("visible")) // Note the optomization here, I found this get me to an acceptable speed
                                    {
                                        ...
                                        IntPtr childPtr = JABHelper.getAccessibleChildFromContext(vmID, currentPtr, i);
                                        ...
                                        //JABHelper.releaseJavaObject(vmID, childPtr);
                                    }
                ...
                finally
                {
                    ...
                    //TODO: Not sure when or where to release the java objects, the JVM will leak memory until released
                    //JABHelper.releaseJavaObject(vmID, currentPtr);
                }
            }
            return null;
        }
        ...
        private static AccessibleTreeItem BuildAccessibleTree(AccessibleContextInfo acInfo, AccessibleTreeItem parentItem, IntPtr elementPointer)
        {
            if (!ReferenceEquals(acInfo, null))
            {
                ...
                {
                    //assign the pointer to be used later to fetch actions
                    item.ElementPointer = (int)elementPointer;
        ...
        public static AccessibleTreeItem GetItemAt(Point mousePt)
        {
            AccessibleTreeItem item = new AccessibleTreeItem();
            screenContents.Sort(new ItemComparer());
            item = screenContents.Find(e => e.Bounds.Contains(mousePt));
            return item;
        }
    }
}
