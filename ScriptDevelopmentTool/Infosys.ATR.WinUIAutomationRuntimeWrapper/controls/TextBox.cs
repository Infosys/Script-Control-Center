...
{
    public class TextBox : Controls.Base.TextBoxBase
    {
        private string className = "TextBox";

        public TextBox(IntPtr appWinHandle, IntPtr screenWinHandle, string automationId, string automationName, string applicationTreePath, string applicationType, string fullControlQualifier)
            : base(appWinHandle, screenWinHandle, automationId, automationName, applicationTreePath, applicationType, fullControlQualifier)
        {
            using (LogHandler.TraceOperations(Logging.InformationMessages.RUNTIMEWRAPPER_ENTER, LogHandler.Layer.Business, Guid.Empty, className, Logging.Constants.TEXTBOX))
            {
                //in param
            ...
            LogHandler.LogInfo(Logging.InformationMessages.RUNTIMEWRAPPER_EXIT, LogHandler.Layer.Business, className, Logging.Constants.TEXTBOX);

        }

        private Condition PrepareCondition()
        {
            PropertyCondition textControl = new PropertyCondition(
            ...
            this.AutomationId);
            AndCondition textControlCondition = new AndCondition(textControl, automationNameProp, automationIdProp);
            return textControlCondition;
        }
    }
}
