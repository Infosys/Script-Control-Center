...
{
    public class Button : Controls.Base.ButtonBase
    {
        private string className = "Button";

        public Button(IntPtr appWinHandle, IntPtr screenWinHandle, string automationId, string automationName, string applicationTreePath, string applicationType, string fullControlQualifier)
            : base(appWinHandle, screenWinHandle, automationId, automationName, applicationTreePath, applicationType, fullControlQualifier)
        {
            Core.Utilities.WriteLog("instantiating button");
            using (LogHandler.TraceOperations(Logging.InformationMessages.RUNTIMEWRAPPER_ENTER, LogHandler.Layer.Business, Guid.Empty, className, Logging.Constants.BUTTON))
            {
                //in param
            ...
            LogHandler.LogInfo(Logging.InformationMessages.RUNTIMEWRAPPER_EXIT, LogHandler.Layer.Business, className, Logging.Constants.BUTTON);
            Core.Utilities.WriteLog("instantiated button");
        }

        private Condition PrepareCondition()
        {
            PropertyCondition buttonControl = new PropertyCondition(
            ...
            this.AutomationId);
            AndCondition buttonControlCondition = new AndCondition(buttonControl, automationNameProp, automationIdProp);
