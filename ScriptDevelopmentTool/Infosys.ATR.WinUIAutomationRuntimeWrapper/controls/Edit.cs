...
{
    public class Edit : Controls.Base.TextBoxBase
    {
        private string className = "Edit";

        public Edit(IntPtr appWinHandle, IntPtr screenWinHandle, string automationId, string automationName, string applicationTreePath, string applicationType, string fullControlQualifier)
            : base(appWinHandle, screenWinHandle, automationId, automationName, applicationTreePath, applicationType, fullControlQualifier)
        {
            using (LogHandler.TraceOperations(Logging.InformationMessages.RUNTIMEWRAPPER_ENTER, LogHandler.Layer.Business, Guid.Empty, className, Logging.Constants.EDIT))
            {
                //in param
            ...
            LogHandler.LogInfo(Logging.InformationMessages.RUNTIMEWRAPPER_EXIT, LogHandler.Layer.Business, className, Logging.Constants.EDIT);
        }

        private Condition PrepareCondition()
        {
            PropertyCondition editControl = new PropertyCondition(
            ...
            this.AutomationId);
            AndCondition editControlCondition = new AndCondition(editControl, automationNameProp, automationIdProp);
            return editControlCondition;
        }
    }
}
