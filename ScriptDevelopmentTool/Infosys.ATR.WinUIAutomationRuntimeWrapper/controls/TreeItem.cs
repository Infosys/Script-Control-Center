...
{
    public class TreeItem : Controls.Base.ListBase
    {
        private string className = "TreeItem";

        public TreeItem(IntPtr appWinHandle, IntPtr screenWinHandle, string automationId, string automationName, string applicationTreePath, string applicationType, string fullControlQualifier)
            : base(appWinHandle, screenWinHandle, automationId, automationName, applicationTreePath, applicationType, fullControlQualifier)
        {
            using (LogHandler.TraceOperations(Logging.InformationMessages.RUNTIMEWRAPPER_ENTER, LogHandler.Layer.Business, Guid.Empty, className, Logging.Constants.TREEITEM))
            {
                //in param
            ...
            LogHandler.LogInfo(Logging.InformationMessages.RUNTIMEWRAPPER_EXIT, LogHandler.Layer.Business, className, Logging.Constants.TREEITEM);

        }

        private Condition PrepareCondition()
        {
            PropertyCondition treeItemControl = new PropertyCondition(
            ...
            this.AutomationId);
            AndCondition treeItemControlCondition = new AndCondition(treeItemControl, automationNameProp, automationIdProp);
            return treeItemControlCondition;
