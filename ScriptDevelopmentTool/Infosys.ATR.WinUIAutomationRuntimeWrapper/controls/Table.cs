...
{
    public class Table: Controls.Base.TableBase
    {
        private string className = "Table";

        public Table(IntPtr appWinHandle, IntPtr screenWinHandle, string automationId, string automationName, string applicationTreePath, string applicationType, string fullControlQualifier)
            : base(appWinHandle, screenWinHandle, automationId, automationName, applicationTreePath, applicationType, fullControlQualifier)
        {
            using (LogHandler.TraceOperations(Logging.InformationMessages.RUNTIMEWRAPPER_ENTER, LogHandler.Layer.Business, Guid.Empty, className, Logging.Constants.DATAGRID))
            {
                //in param
            ...
            LogHandler.LogInfo(Logging.InformationMessages.RUNTIMEWRAPPER_EXIT, LogHandler.Layer.Business, className, Logging.Constants.DATAGRID);

        }

        private Condition PrepareCondition()
        {
            PropertyCondition tabletemControl = new PropertyCondition(
            ...
            this.AutomationId);
            AndCondition buttonControlCondition = new AndCondition(tabletemControl, automationNameProp, automationIdProp);
