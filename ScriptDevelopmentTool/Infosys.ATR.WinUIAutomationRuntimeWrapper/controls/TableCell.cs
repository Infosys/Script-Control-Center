...
{
    public class TableCell: Controls.Base.CustomBase
    {
        private string className = "TableCell";

        public TableCell(IntPtr appWinHandle, IntPtr screenWinHandle, string automationId, string automationName, string applicationTreePath, string applicationType, string fullControlQualifier)
            : base(appWinHandle, screenWinHandle, automationId, automationName, applicationTreePath, applicationType, fullControlQualifier)
        {
            using (LogHandler.TraceOperations(Logging.InformationMessages.RUNTIMEWRAPPER_ENTER, LogHandler.Layer.Business, Guid.Empty, className, Logging.Constants.CUSTOM))
            {
                //in param
            ...
            LogHandler.LogInfo(Logging.InformationMessages.RUNTIMEWRAPPER_EXIT, LogHandler.Layer.Business, className, Logging.Constants.CUSTOM);

        }

        private Condition PrepareCondition()
        {
            PropertyCondition customControl = new PropertyCondition(
            ...
            this.AutomationId);
            AndCondition controlCondition = new AndCondition(customControl, automationNameProp, automationIdProp);
        ...
        public string ReceiveText()
        {
            ...
            {
                text = this.Control_Reference.WinControlReference.Current.Name;
            }
            return text;
        }
    }
