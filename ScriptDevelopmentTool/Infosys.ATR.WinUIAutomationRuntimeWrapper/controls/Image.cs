...
{
    public class Image : Controls.Base.ImageBase
    {
        private string className = "Image";

        public Image(IntPtr appWinHandle, IntPtr screenWinHandle, string automationId, string automationName, string applicationTreePath, string applicationType, string fullControlQualifier)
            : base(appWinHandle, screenWinHandle, automationId, automationName, applicationTreePath, applicationType, fullControlQualifier)
        {
            using (LogHandler.TraceOperations(Logging.InformationMessages.RUNTIMEWRAPPER_ENTER, LogHandler.Layer.Business, Guid.Empty, className, Logging.Constants.IMAGE))
            {
                //in param
            ...
            LogHandler.LogInfo(Logging.InformationMessages.RUNTIMEWRAPPER_EXIT, LogHandler.Layer.Business, className, Logging.Constants.IMAGE);

        }

        private Condition PrepareCondition()
        {
            PropertyCondition imageControl = new PropertyCondition(
            ...
            this.AutomationId);
            AndCondition buttonControlCondition = new AndCondition(imageControl, automationNameProp, automationIdProp);
