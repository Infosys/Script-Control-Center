...
{
    public class Tab: Controls.Base.TabBase
    {
        private string className = "Tab";

        public Tab(IntPtr appWinHandle, IntPtr screenWinHandle, string automationId, string automationName, string applicationTreePath, string applicationType, string fullControlQualifier)
            : base(appWinHandle, screenWinHandle, automationId, automationName, applicationTreePath, applicationType, fullControlQualifier)
        {
            using (LogHandler.TraceOperations(Logging.InformationMessages.RUNTIMEWRAPPER_ENTER, LogHandler.Layer.Business, Guid.Empty, className, Logging.Constants.TAB))
            {
                //in param
            ...
            LogHandler.LogInfo(Logging.InformationMessages.RUNTIMEWRAPPER_EXIT, LogHandler.Layer.Business, className, Logging.Constants.TAB);

        }

        private Condition PrepareCondition()
        {
            PropertyCondition tabControl = new PropertyCondition(
            ...
            this.AutomationId);
            AndCondition buttonControlCondition = new AndCondition(tabControl, automationNameProp, automationIdProp);
