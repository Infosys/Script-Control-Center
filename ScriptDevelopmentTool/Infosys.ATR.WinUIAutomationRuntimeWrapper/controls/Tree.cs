...
{
    public class Tree : Controls.Base.ListBase
    {
        private string className = "Tree";

        public Tree(IntPtr appWinHandle, IntPtr screenWinHandle, string automationId, string automationName, string applicationTreePath, string applicationType, string fullControlQualifier)
            : base(appWinHandle, screenWinHandle, automationId, automationName, applicationTreePath, applicationType, fullControlQualifier)
        {
            using (LogHandler.TraceOperations(Logging.InformationMessages.RUNTIMEWRAPPER_ENTER, LogHandler.Layer.Business, Guid.Empty, className, Logging.Constants.TREE))
            {
                //in param
            ...
            LogHandler.LogInfo(Logging.InformationMessages.RUNTIMEWRAPPER_EXIT, LogHandler.Layer.Business, className, Logging.Constants.TREE);

        }

        private Condition PrepareCondition()
        {
            PropertyCondition treeControl = new PropertyCondition(
            ...
            this.AutomationId);
            AndCondition treeControlCondition = new AndCondition(treeControl, automationNameProp, automationIdProp);
            return treeControlCondition;
