...
namespace Infosys.ATR.ScriptEditor
{
    public partial class ActivityView : UserControl
    {
        ...
        public ActivityView(Activity activity, string useCaseId)
        {
            InitializeComponent();
            _activity = activity;
            _useCaseId = useCaseId;
            txtActId.Text = !string.IsNullOrEmpty(activity.Name) ? activity.Name : activity.Id;
            ...
            //txtActTags.Text = activity.Tags ?? "";
            //if (!string.IsNullOrWhiteSpace(activity.Tags))
            {
                if (string.IsNullOrWhiteSpace(activity.Tags))
                    activity.Tags = "";
                string[] tags = activity.Tags.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        void ActivityView_Paint(object sender, PaintEventArgs e)
        {
            ...
            y = lblSelectedTask.Location.Y - 5;
            e.Graphics.DrawLine(pen, x, y, x + 640, y);

            //last horizontal line
            x = lblEnd.Location.X;
            y = lblEnd.Location.Y - 5;
            e.Graphics.DrawLine(pen, x, y, x + 640, y);
        }
        ...
        private void dgTasks_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(imagePath))
            {
                ...
                if (System.IO.File.Exists(imagefile))
                {
                    ...
                    {
                        pic.Image = img.Clone(new Rectangle(0, 0, img.Width, img.Height), img.PixelFormat);
            ...
            if (clickCounter == 2)
            {
                ...
                else
                {
                    ...
                    if (pic.Bounds.Contains(cropImageLocation))
                    {
                        ...
                        System.Drawing.Image newImage = CropImage(pic.Image, cropImage);
                        pic.Image = newImage;
                    }
                }
            }
        ...
        private System.Drawing.Image CropImage(System.Drawing.Image img, Rectangle rect)
        {
            imageChanged = true;
        ...
        {
            ContextMenu taskTypes = new ContextMenu();
            taskTypes.MenuItems.Add(new MenuItem("Text Entry"));
            taskTypes.MenuItems.Add(new MenuItem("Pause for User Input"));
            taskTypes.MenuItems.Add(new MenuItem("Shift Key Press"));
            taskTypes.MenuItems.Add(new MenuItem("Control Key Press"));
            taskTypes.MenuItems.Add(new MenuItem("Caps Lock Key Press"));
            taskTypes.MenuItems.Add(new MenuItem("Print Screen Key Press"));
            taskTypes.MenuItems.Add(new MenuItem("Mouse Click"));
            taskTypes.MenuItems.Add(new MenuItem("Child Use Case"));
            btnAddTask.ContextMenu = taskTypes;
            taskTypes.Show(btnAddTask, new Point());
        }
    }
}
