...
"ltr"?c.tblwidth+g:c.tblwidth-g;d.newWidth=h}}}},dragEnd:function(){this.hDiv.style.cursor="default";if(this.resizing){var d=this.resizing.idx,g=this.headers[d].newWidth||this.headers[d].width;g=parseInt(g,10);this.resizing=false;b("#rs_m"+c.id).css("display","none");c.colModel[d].width=g;this.headers[d].width=g;this.headers[d].el.style.width="100px";this.cols[d].style.width="100px";if(this.footers.length>0)this.footers[d].style.width="100px";if(c.forceFit===true){g=this.headers[d+c.nv].newWidth||this.headers[d+
c.nv].width;this.headers[d+c.nv].width=g;this.headers[d+c.nv].el.style.width="100px";this.cols[d+c.nv].style.width="100px";if(this.footers.length>0)this.footers[d+c.nv].style.width="100px";c.colModel[d+c.nv].width=g}else{c.tblwidth=this.newWidth||c.tblwidth;b("table:first",this.bDiv).css("width",c.tblwidth+"px");b("table:first",this.hDiv).css("width",c.tblwidth+"px");this.hDiv.scrollLeft=this.bDiv.scrollLeft;if(c.footerrow){b("table:first",this.sDiv).css("width",c.tblwidth+"px");this.sDiv.scrollLeft=
...
a("td:eq("+(S-1)+")",K[0]).append("<span>"+V+"</span>").attr("id","v_"+O);B&&a("td:eq("+(S-1)+") span",K[0]).css({"text-align":"right",width:"100px"});$[N]=A;N++}});if(N>0){i=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(R*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='id' value='"+i+"'/></td></tr>");i[0].rp=N+99;a(J).append(i)}return $}function y(i,f){var J,R,O=0,T,K;if(K=a(f).jqGrid("getInd",i,true)){a("td",K).each(function(N){J=
